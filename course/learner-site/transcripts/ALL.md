...
- [M2 — The On-Device AI App: Architecture](#m02) — 26 slides, 14m 1s
...
**26 slides · 26 narrated · 14m 1s of audio**
...
> ConversationStore is the single source of truth. It keeps a log of finals plus one current partial. Applying a result appends finals and replaces partials. The context window walks utterances newest-first, keeping each one while it fits, and it always keeps the newest even if that one alone exceeds the budget, so the window is never empty. The default budget is 4,000 characters, but recap and action-item prompts get 100,000, because they have to cover the whole conversation.
...
*32.0s · sentence-measured*

> This is the first proof slide, and it sets a pattern for the course. Every claim we make has a file pointer you can open, and that pointer resolves. The guarantee here is that the context window is never empty: the newest-first fit always keeps the most recent segment, even when it alone blows the budget. The default budget of 4,000 characters lives in ContextEngine. Open the file and read the loop yourself rather than trusting the bullets. The 96% core coverage badge and the 95% floor are downstream consequences of this kind of layering.
...
> Here is a small check with a large blast radius. The router splits the model name into tokens and lets a marker match only at a token start. Look at gemini-2.5-flash: it contains the letters m-i-n-i, so a raw substring check would call it mini and rank it as fast. The token-prefix check rejects that hit, because mini is not a token in that name. One wrong check misroutes an entire model family, and no test fails unless you wrote the near-miss test. TinyCopilot ships exactly that test: test_gemini_is_not_demoted_as_mini.
...
**28 slides · 28 narrated · 18m 0s of audio**
...
> The spec file owns the WHAT. In the security-hardening spec you will find eight user stories, six of them P1 and two P2, forty-four functional requirements all phrased System MUST, seven edge cases, and twelve success criteria. Open story one: given a user attempts to log in, when they fail authentication five times within five minutes, then further login attempts are blocked for fifteen minutes. Those numbers, five, five, and fifteen, will reappear in a functional requirement, in a contract config row, and in a test assertion. That recurrence is the point. And the spec names no technology at all.

### Slide 5 — Proof: `research.md` — decisions with receipts

*43.4s · sentence-measured*

> Research settles technology, and it does so with receipts. This file is nine hundred sixty-five lines holding eight numbered decisions, each with the same anatomy: the decision, the options evaluated with pros and cons, the rationale, then implementation details. Decision one chooses Redis and rejects in-memory outright, because in-memory counters are lost on restart and are not shared across API instances. Decision two chooses TOTP, time-based one-time passwords, over SMS, because NIST special publication 800-63B deprecates SMS and SIM-swap attacks are well documented. Here is the rule to steal: a decision without a rejected alternative is a preference, not a decision.

### Slide 6 — Proof: `plan.md` — HOW, plus one absence

*36.2s · sentence-measured*

> The plan owns the HOW, and it opens with a gate. The Constitution Check walks the project's seven principles and gives each one a verdict, ending with the line Constitution Violations: NONE and an empty Complexity Tracking table, which the template says to fill only when violations exist. Every file the plan touches is annotated as new or modify. Then the honest absence: this feature has no data-model file, because security infrastructure spans many entities, and the plan says so. Six other specs do have one. The template defines the full set of artifacts, but each feature decides what earns its keep.

### Slide 7 — Proof: `contracts/` — six seams

*38.2s · sentence-measured*

> The contract folder is where WHAT and HOW first meet. This feature has six contracts, and together they run to four thousand six hundred sixty-one lines: rate limiting, the two-factor API, audit logging, CSRF, session, and password reset. Look at the rate-limiting config table's first row: a POST to the login endpoint, a five minute window, five attempts, per IP, with a fifteen minute lockout — the same five, five, fifteen from the acceptance scenario, now with a scope and a key format attached. That is what a contract is for. If you write one artifact carefully in the lab, make it the contract.

### Slide 8 — Proof: the gate and the quickstart

*35.9s · sentence-measured*

> The quickstart is six hundred forty-three lines of timed deployment with exact commands, plus prerequisites and troubleshooting. The requirements checklist is fifty lines and grades three groups: Content Quality, Requirement Completeness, and Feature Readiness. Its first rule bans implementation details, and it ends with a verdict of all checks passed and a quality score of one hundred percent. Hold that number. In a few slides I will show you a count in this very file that is wrong, and the one hundred percent did not catch it. A self-graded gate tells you what its author believed; only reading it against the spec tells you what is true.

### Slide 9 — Proof: `tasks.md` — Phase 2, and the gap

*38.8s · sentence-measured*

> Here is the honest gap in the exemplar. This feature was specced down to four thousand lines of contracts and never generated a task breakdown; its plan's Next Steps still lists that command as Phase 2. So read the real format from the template: checkboxes with an ID, an optional parallel marker, and a story tag. The parallel marker flags work that can run at the same time, the story tag ties each task to a story, and the template requires exact file paths in descriptions. One real line reads: T017, parallel, story one, implement the create wizard state method in the onboarding service file. Every task names a file.
...
**28 slides · 28 narrated · 16m 7s of audio**
...
> Read two records and the difference becomes visible. The METR entry says AI-allowed issues took nineteen percent longer, with a confidence interval from plus two to plus thirty-nine percent, across two hundred and forty-six real issues. Developers expected twenty-four percent faster and afterwards believed twenty percent faster. Two entries down, Peng reports fifty-five point eight percent faster, with ninety-five freelancers on one synthetic task, and that work was vendor-affiliated. METR was independent. Both results are real, and the record structure refuses to average them.
...
> Three files exist only to record where content came from. The document manifest lists eleven retrievals, each with a URL, a retrieval date, a status and a SHA-256 hash of the downloaded file. Entry M02 is the McKinsey PDF, and its status is unavailable, with the reason recorded word for word: the read operation timed out. A spotless manifest is suspicious. A manifest that logs its own failure is checkable, because you can re-run the retrieval. Even images have provenance: the visual-provenance file records that the two hero illustrations are conceptual, not photographs of deployed systems, and it keeps the prompts.
...
> Content needs tests, so the check target is models, build, site, browser. The models step runs the npm tests, the Python unit tests, narration validation with require-complete, and contract validation. The browser step runs five CI-identical groups — playback, flows, site, models and architecture — in both Chromium and WebKit, at three viewports: twelve eighty by seven twenty, nineteen twenty by ten eighty, and three seventy-five by eight twelve. A slide that overflows a projector viewport fails like a unit test. And the link check does not treat a successful HTTP response as evidence that a claim is correct.

### Slide 22 — Hash-gate the media

*34.0s · sentence-measured*

> One hundred and sixteen slides carry recorded narration, so a silent slide edit would desynchronize the voice from the slide. The narration-review tool hashes each rendered slide together with its script, its recording and its caption-flow definitions. If a destination changes, you must make a deliberate retained or refreshed decision, with a specific reason, recorded in the review data. Here is the line to carry into your own pipeline: never reset the baseline to silence a stale-review failure. Resetting the baseline does not fix the drift; it rubber-stamps the drift you just erased.
...
**28 slides · 28 narrated · 15m 52s of audio**
...
> This is the file where the pattern is recorded: the competition analysis document, holding 12 competitor rows dated August 2026. Note the header convention. Every price fact is stated as of that date, and where a detail could not be confirmed from a primary source it is qualified as approximately or reportedly. The price column carries the model, the price, and the channel, and commercial tiers run about $8 to $149 a month. This is not a summary; it is evidence you can audit. Copy the convention in Lab M7, so every price cell gets a source URL and a retrieval date.
...
> Here is the rule that connects the two prices. Self-paced should be priced as 70 to 85% of the live cohort price, but only if it keeps projects plus async feedback or office hours plus community. Strip all three and the research verdict is blunt: a stack of Zoom recordings is not a self-paced course. A bare video library should not be sold at all. Generalize that test. A stack of features is not an app tier, and a stack of notes is not a briefing. The question that tests any package is: what can the buyer show for the money?
...
> This is the course's own arithmetic. Walk the lines: the course-platform fee, community at about $49 a month or zero on Discord, and email at about $29 a month, giving roughly $80 to $130 a month in fixed cost plus per-enrollment fees. At the $399 self-paced tier, that is about two sales a month to break even. The transferable move is the order: floor, then comparator band, then value anchor, then price. A price below your floor is a subsidy. SignUpFlow's SQLite-in-dev, Postgres-in-prod ladder is the same discipline for a software product.
