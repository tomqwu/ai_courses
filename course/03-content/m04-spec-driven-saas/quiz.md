...
- c) So the path can be grepped to confirm it exists
...
Name the two hard lines the course quotes about review and approval, and explain in one sentence each what they forbid.

### Q8 (M4.1/M4.3, short answer)
...
- **Q1 — b.** WHAT/HOW separation: `spec.md` owns user-visible behavior technology-agnostically; `plan.md` owns HOW. The gate's first rule enforces it ("No implementation details", `specs/014-security-hardening/checklists/requirements.md`). (M4.1, M4.2)
...
- **Q5 — d.** Ties to the anti-hallucination rule: "Do not invent file paths… Grep the repo before referencing" (`AGENTS.md`). A confirmed path makes the agent's first action a grep, not a guess. (M4.2)
...
- **Q7 —** (1) "Do not claim independent review when the builder performed the review itself" — forbids self-review masquerading as independent review; (2) "Missing review is not approval" (`AGENTS.md`, PR rule 4) — forbids merging on absent evidence or treating no-review as a pass. (M4.3)
