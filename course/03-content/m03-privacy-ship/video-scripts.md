...
| 3:45–4:45 | `OllamaProvider.swift:138-157` | Three layers around it. The host must be localhost, 127.0.0.1, or ::1, checked before a byte of prompt is written. Verification re-runs on every request, so switching models mid-session cannot skip it. And redirects are refused. |
| 4:45–5:30 | `OllamaProvider.swift:138-142, 208-214` | Redirect following is silent by default. Even a verified-local server could answer with a 3xx, and the stack would forward your meeting text. `RejectRedirects` answers nil and the request dies. Never follow redirects with meeting text in local-only mode. |
